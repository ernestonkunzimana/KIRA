...
LOG_FILE = "/app/src/detection_metrics.json"
MODEL_CACHE = "/app/src/edge_model.pkl"
...
def publish_detection_event(timestamp, node_id, metrics, ground_truth, prediction, score, decision):
    """Forward detection events to the enterprise backend API."""
    url = f"{BACKEND_API_URL}/api/v1/events/detection"
    payload = {
        "timestamp": timestamp,
        "node_id": node_id,
        "metrics": {
            "vibration_hz": metrics.get("vibration_hz", 0),
            "temperature_c": metrics.get("temperature_c", 0),
            "link_quality_qos": metrics.get("link_quality_qos", 100),
        },
    ...
    try:
        ...
        # Keep inference path non-blocking even if API is unavailable.
        pass
        ...
        with state_lock:
            if not IS_TRAINED:
                ...
                if progress >= TRAINING_THRESHOLD:
                    train_edge_model(training_buffer)
            else:
                # ================================================================
                ...
                publish_detection_event(timestamp, node_id, metrics, ground_truth, prediction, score, decision)
